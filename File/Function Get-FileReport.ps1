...
http://77.104.138.174/~powershe/power-shell.com
 #>
    ...
    try
    {
        ...
        If ($Ext)
        {
            $files = Get-ChildItem -path $path -recurse -filter *.$ext -ErrorAction stop | Where-Object {$_.CreationTime -lt $olderthan}
        }
        else
        {
            $files = Get-ChildItem -Path $path -Recurse -ErrorAction stop | Where-Object {!$_.PSIsContainer -and $_.CreationTime -lt $olderthan}
        }
        ...
        foreach ($file in $files)
        {
            ...
            $fileObject |Sort-Object Last_Change_Date #Output to the screen for a visual feedback
            $fileColl += $fileObject #Copy the contents of the object into the Array
            $fileObject = $null #Delete the fileObject
        } #end foreach
        If ($fileColl)
        {
            Write-Output "File properties successfully retrieved."
        }
        If ($export)
        {
    ...
    catch
    {
        If (!(Test-Path $Path))
        {
            Write-Output "Cannot find ""$path"" because it does not exist."
        }
        If (!($files))
        {
            Write-Output "No $Ext files found older than $olderthan."
        }
        If (($Export) -and (!(Test-Path $Export)))
        {
            Write-Output "Cannot export data to ""$Export"" because it does not exist."
        }
    }
    If (Test-Path $export\$HostName-Files-Older-Than-$csvname.csv)
    {
        Write-Output "$export\$HostName-Files-Older-Than-$csvname.csv created successfully."
    }
