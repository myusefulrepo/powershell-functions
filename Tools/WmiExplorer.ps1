        ...
        $MOC |
            ...
            $MO.psbase.properties |
                ...
                Else
                {
                    ...
                    trap
                    {
                        $DR.Item("$($prop.Name)*") = $prop.Value.tostring(); continue
                    }
                }
...
Function GetClassesFromNameSpace
{
    ...
    if ($this.SelectedNode.name -ne $script:computer)
    {
        ...
        if ($Script:nsc.Item("$mp"))
        {
            #in Namespace Cache, so just attach to ListView again
        ...
        {
            #Not In NameSpace Cache

            if (${Global:WmiExplorer.dtClasses}.Select("Namespace = '$mp'"))
            {
                # In DataTable cache, so get from there
            ...
            {
                # Not in any Cache , Load WMI Classes
                ...
                $Mc.psbase.GetSubclasses($eo) |
                    ...
                    Trap
                    {
                        $script:Description = "[Empty]"; continue
                    }
...
Function GetWmiClass
{
    ...
    $script:WmiClass.psbase.properties |
        ...
        & {
            Trap
            {
                $rtbHelp.appendtext("[Empty]"); Continue
            }
...
Function OutputWmiInstance
{
    ...
    {
        if (-not $Script:InstanceTab)
        {
            $Script:InstanceTab = new-object System.Windows.Forms.TabPage
...
Function GetWmiMethod
{
    ...
    $q = $script:WmiClass.PSBase.Methods[$WMIMethod].Qualifiers | ForEach-Object {$_.name}
    ...
    if ($true)
    {
        trap
        {
            $rtbMethods.AppendText('[None]') ; continue
        }

        $inParam.PSBase.Properties | ForEach-Object {
            $Q = $_.Qualifiers | ForEach-Object {$_.name}
            ...
            if ($q -contains "Description")
            {
                $rtbMethods.AppendText($_.Qualifiers["Description"].Value)
            }
            $HasParams = $true
        }
    }
...
trap
{
    Write-Host $_ ; $status.Text = "unexpected error"; $slMessage.Text = "$_.message"; continue
}
